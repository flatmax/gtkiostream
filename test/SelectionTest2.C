...
int main(int argc, char *argv[]) {
    ...
    SelectionColumn sct("text column", 0); // create the columns class with a new column which handles text
    sct<<pair<string*, GtkCellRendererText*>(new string("text"), GTK_CELL_RENDERER_TEXT(gtk_cell_renderer_text_new())); // create a column for text
    SelectionColumn scp("pixmap col", 1); // create the pixmap column
    scp<<pair<string*, GtkCellRendererPixbuf*>(new string("pixbuf"), GTK_CELL_RENDERER_PIXBUF(gtk_cell_renderer_pixbuf_new())); // create a second column which handles pixbufs
    selection<<&sct; // setup the Selection with the columns
