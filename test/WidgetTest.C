...
static void saveToImage(GtkWidget *wid, gpointer data){
	Widget widget(wid);
	widget.saveToPNGFile("widget.png");
}
...
int main(int argc, char *argv[]){
  ...
  buttons<<BUTTONLABELSTRUCT("Quit", quit, &buttons)<<BUTTONTOGGLESTRUCT("TOGGLE",toggleButton, 0) // normal and toggle buttons
	     <<BUTTONLABELSTRUCT("Save to image", reinterpret_cast<GCallback>(saveToImage), NULL) // normal and toggle buttons
         <<BUTTONPIXMAPSTRUCTTRANSPARENT(playXPM, reinterpret_cast<GCallback>(playButton), 0) // transparent pixmap button
         <<BUTTONPIXMAPSTRUCT(playXPM, reinterpret_cast<GCallback>(playButton), 0); // regular buttons are the same
