...
void ImpulsePinkInv<FP_TYPE>::setMag(Array<typename FFT<double>::Complex, Dynamic, 1> const &X, float fs, float fi, float fa){
  ...
  sf=sf.sqrt();
  // std::cout<<sf.abs().maxCoeff()<<std::endl;
...
template class ImpulsePinkInv<short int>;
template class ImpulsePinkInv<int>;
template class ImpulsePinkInv<float>;
template class ImpulsePinkInv<double>;
